...
import com.sshs.core.util.*;
import com.sshs.shiro.error.SecurityErrorCode;
import com.sshs.shiro.mapper.LoginUserMapper;
import com.sshs.shiro.model.LoginModel;
...
public class LoginController {
    ...
    @ResponseBody
    public Message<JSONObject> login(@RequestBody LoginModel sysLoginModel) {
      ...
      /*  String captcha = sysLoginModel.getCaptcha();
       ...
       /* String lowerCaseCaptcha = captcha.toLowerCase();
        ...
        //1. 校验用户是否有效
        List<LoginUser> sysUsers = loginUserMapper.findSecurityUserByUserName(username);
        ...
        LoginUser sysUser = sysUsers.get(0);
        String salt = sysUser.getSalt();
        if (StringUtils.isBlank(salt)) {
            salt = sysUser.getUsername();
        }
        //2. 校验用户名或密码是否正确
        String userpassword = PasswordUtil.encrypt(username, password, salt);
        String syspassword = sysUser.getPassword();
       /* if (!syspassword.equals(userpassword)) {
            throw new BusinessException(SecurityErrorCode.PASSWORD_IS_WRONG);
        }*/
    ...
    public Message<Object> logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (username != null) {
            ...
            return Message.failure(-999, "Token无效!");
        }
    }
...
*/
     ...
     * @return
     */
    private Message<JSONObject> userInfo(LoginUser sysUser) {
        String syspassword = sysUser.getPassword();
        ...
        ContextUtil.getSession().setAttribute(JwtUtil.X_ACCESS_TOKEN, token);
        // 获取用户部门信息
        JSONObject obj = new JSONObject();
        //List<SysDepart> departs = sysDepartService.queryUserDeparts(sysUser.getId());
        List<String> departs = new ArrayList<>();
        departs.add(sysUser.getOrgCode());
   ...
   /* @GetMapping(value = "/getEncryptedString")
    public Message<Map<String, String>> getEncryptedString() {
    ...
    public Message<Map<String, String>> getCheckCode() {
        //Result<Map<String, String>> result = new Result<Map<String, String>>();
        Map<String, String> map = new HashMap<String, String>();
        try {
            ...
            e.printStackTrace();
            throw new BusinessException(SecurityErrorCode.AUTHORISE_INVALID);
        }
    }
    ...
    public Message<String> randomImage(HttpServletResponse response, @PathVariable String key) {
        try {
            String code = RandomUtil.randomString(BASE_CHECK_CODES, 4);
            ...
            return Message.success(base64);
        } catch (Exception e) {
            e.printStackTrace();
            throw new BusinessException(-999, "获取验证码出错" + e.getMessage());
        }
    }
    ...
    public Result<JSONObject> mLogin(@RequestBody LoginModel sysLoginModel) throws Exception {
        Result<JSONObject> result = new Result<JSONObject>();
        String username = sysLoginModel.getUsername();
        String password = sysLoginModel.getPassword();
